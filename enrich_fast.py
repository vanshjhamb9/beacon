"""Fast 200-lead enrichment — 1 page per site, 5s timeout, direct DB writes."""
import re, uuid, asyncio, os
...
DISPOSABLE = {"tempmail.com","throwaway.email","guerrillamail.com","mailinator.com","yopmail.com","trashmail.com","10minutemail.com","temp-mail.org"}
...
LEADS = [
    ...
    ("Core Fitness","https://corefitness.in","fitness","Gym"),("Domino's India","https://dominos.co.in","restaurant","QSR"),
    ("McDonald's India","https://mcdonalds.co.in","restaurant","QSR"),("Subway India","https://subway.com/in","restaurant","QSR"),
    ("KFC India","https://kfc.co.in","restaurant","QSR"),("Pizza Hut India","https://pizzahut.co.in","restaurant","QSR"),
...
async def scrape(client, url):
    ...
    pages = [url, url.rstrip("/")+"/contact-us", url.rstrip("/")+"/contact", url.rstrip("/")+"/pages/contact-us"]
    for p in pages:
        try:
            r = await client.get(p, follow_redirects=True, timeout=5)
...
async def main():
    ...
    async with engine.begin() as conn:
        await conn.execute(text("DELETE FROM ecommerce_leads WHERE deleted_at IS NULL"))
        print("Cleared old leads")

    seen_sites = set()
    unique = []
    for l in LEADS:
        d = urlparse(l[1]).netloc.lower().replace("www.","")
        if d not in seen_sites:
            seen_sites.add(d)
            unique.append(l)
    ...
    async with httpx.AsyncClient(headers=HEADERS, verify=False, follow_redirects=True) as client:
        for i, (name, url, ind, cat) in enumerate(unique):
            emails, phones = await scrape(client, url)
            has_e, has_p = bool(emails), bool(phones)
            if has_e: with_email += 1
            if has_p: with_phone += 1
            if has_e and has_p: with_both += 1

            entry = {"name":name,"url":url,"industry":ind,"category":cat,"emails":emails,"phones":phones}
            ...
            if emails or phones:
                ...
                async with engine.begin() as conn:
                    ...
                    """),{"id":str(uuid.uuid4()),"name":name,"url":url,"domain":domain,"ind":ind,"cat":cat,"email":email or "","phone":phone or "","conf":0.8 if has_e and has_p else 0.5,"sc":sc,"pri":pri})

            mark = "OK" if has_e or has_p else "---"
            print(f"[{i+1:3d}/{len(unique)}] {name:30s} E:{len(emails)} P:{len(phones)} {mark}")

            if (i+1)%20==0:
                await asyncio.sleep(1)
                print(f"  >>> Progress: {i+1}/{len(unique)} | Email:{with_email} Phone:{with_phone} Both:{with_both}")
